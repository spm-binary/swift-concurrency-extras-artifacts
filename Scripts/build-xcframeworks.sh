...
upstream_url="${UPSTREAM_URL:-https://github.com/pointfreeco/swift-concurrency-extras.git}"

products=(ConcurrencyExtras)
platforms=(
...
work_dir="${RUNNER_TEMP:-/tmp}/swift-concurrency-extras-${release_tag}"
...
for manifest in Package.swift Package@swift-6.0.swift; do
  if [[ -f "$manifest" ]]; then
    perl -0pi -e 's/\.library\(\s*\n\s*name: "ConcurrencyExtras",\s*\n\s*targets: \["ConcurrencyExtras"\]\s*\n\s*\)/.library(name: "ConcurrencyExtras", type: .dynamic, targets: ["ConcurrencyExtras"])/g' "$manifest"
  fi
done
